...
while sec < (loop_time * 60):
    ...
    ######################################################################
    ## 매매할 종목 설정
    market_code_list = ['KRW-BTC', 'KRW-ETH', 'KRW-XRP']
    # ==========================================================================================
    ...
    for market_code in market_code_list:
        ...
        if now_krw > 10000:
            ...
            if RSI_value_number <= down_bound:
                ...
                if(is_already_bought == False):
                    ...
                    send_email(f'{market_code} RSI 지표가 {down_bound}% 이하 과매도 상태 시장가 매수 ' + datetime.datetime.now().strftime('%Y-%m-%d %H시 %M분'), "🧾 매수했어요")
                    print(f'{market_code} RSI 지표가 {down_bound}% 이하 과매도 상태 시장가 매수 ' + datetime.datetime.now().strftime('%Y-%m-%d %H시 %M분'))
            ...
            ## RSI 지표가 up_bound% 이상이면 들여 쓴 코드 실행
                ...
                if pyupbit.Upbit(A_key, S_key).get_balance(market_code) > 0:
                    ...
                    send_email(f'{market_code} RSI 지표가 {up_bound}% 이상 과매수 상태 시장가(수익화) 매도 ' + datetime.datetime.now().strftime('%Y-%m-%d %H시 %M분'), "🏷️ 매도했어요")
                    print(f'{market_code} RSI 지표가 {up_bound}% 이상 과매수 상태 시장가(수익화) 매도')
                ## 종목 보유량이 없는 경우 들여 쓴 코드 실행
                    ...
                    print(f'{market_code} RSI 지표가 {up_bound}% 이상 과매수 상태지만 매도할 종목 보유량 없음')
        ...
        ## 보유 원화가 10000원 미만이면 들여 쓴 코드 실행
            ...
            if pyupbit.Upbit(A_key, S_key).get_balance(market_code) > 0:
                ...
                if RSI_value_number <= down_bound:
                    send_email(f'{market_code} RSI 지표가 {down_bound}% 이하 과매도 상태지만 매수할 원화 부족 ' + datetime.datetime.now().strftime('%Y-%m-%d %H시 %M분'), "❌ 매수못했어요")
                    print(f'{market_code} RSI 지표가 {down_bound}% 이하 과매도 상태지만 매수할 원화 부족')
                ## RSI 지표가 up_bound% 이상이면 들여 쓴 코드 실행
                elif RSI_value_number >= up_bound:
                    ################################################################################
                    ...
                    send_email(f'{market_code} RSI 지표가 {up_bound}% 이상 과매수 상태 시장가(수익화) 매도 ' + datetime.datetime.now().strftime('%Y-%m-%d %H시 %M분'), "🏷️ 매도했어요")
                    print(f'{market_code} RSI 지표가 {up_bound}% 이상 과매수 상태 시장가 매도')
